...
mixin ArgsMixin {
  final List<String> _args = HpfCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(HpfCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return HpfCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < HpfCli.arguments.length) {
      index++;
      return HpfCli.arguments[index];
    } else {
